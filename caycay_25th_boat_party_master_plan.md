# CayCay's 25th Birthday Boat Party - Master Plan

> **Purpose:** Single source of truth for planning, assigning, and
> executing CayCay's 25th birthday boat party.\
> **Event goal:** A fun, relaxed, well-organized birthday where CayCay
> feels celebrated and the hosts can actually enjoy the party.\
> **Planning style:** Minimal structure, maximum vibes. Handle logistics
> in advance so the two hours onboard feel effortless.

------------------------------------------------------------------------

## 1. Event Snapshot

  -----------------------------------------------------------------------
  Item                                Confirmed Detail
  ----------------------------------- -----------------------------------
  Occasion                            CayCay's 25th birthday

  Date                                Saturday, October 3, 2026

  Cruise time                         **12:00 PM-2:00 PM**

  Departure                           **Bass Tub, 276 Jefferson Street,
                                      Berth #4, Fisherman's Wharf, San Francisco**

  Boat                                San Francisco Bay Yacht Cruise

  Captain / boat contact              **Aaron**

  Boat capacity                       55 people

  Partiful guest limit                50

  Current RSVPs                       **38 Going + 11 Maybe**

  Total paid                          **\$1,376.04**

  Boat                                \$1,200

  Service fee                         \$176.04

  Cancellation                        Free until 7 days before trip

  Guest contribution                  **\$20 suggested**

  Main communication                  **Partiful** - bulk text/app
                                      notifications

  Party style                         No formal run of show - **just
                                      vibes**, with a few intentional
                                      birthday moments
  -----------------------------------------------------------------------

### Cost Reference

-   38 people x \$20 = **\$760** contributed -\> hosts cover
    **\$616.04**
-   50 people x \$20 = **\$1,000** contributed -\> hosts cover
    **\$376.04**
-   Decision: **Keep the suggested contribution at \$20.**

------------------------------------------------------------------------

## 2. Guest Arrival Plan

### Updated Arrival Plan

**\~11:00 AM - Gio + supply car** - Working plan: park at **Pier 39
Garage**, then use Gio's brother's folding cart for the roughly
**7-minute / 0.3-mile walk** to the boat. This garage is Gio's practical
supply plan, not yet a blanket parking recommendation for guests.

**11:00-11:30 AM - No formal guest meetup** - This window belongs to
Gio and the helpers for parking, moving supplies, and getting organized.
Early guests can explore Fisherman's Wharf or grab food nearby on their
own, then meet everyone at the boat at 11:45. Do not create another
official gathering point or transition for the hosts to manage.

**\~11:10-11:30 AM - Supply move / host arrival** - Bring pizza, drinks,
cake, decor, and other supplies to **Bass Tub, Berth #4**. Aaron has not
promised access that early, so initially this may only mean getting
everything to the dock. Board only when Aaron and the crew are ready.

**11:45 AM - Official guest arrival / ready to board** - Everyone should
be at **Bass Tub, 276 Jefferson Street, Berth #4** by 11:45. This exact
berth is the clear anchor so nobody is confused or left at a bar.

**11:45 AM - Boarding readiness** - Confirmed working instruction:
guests should be ready to get on the boat at 11:45 AM.

**12:00 PM - Cruise starts** - Do not advertise a fake departure time. -
Final guest messaging should make clear that late arrival risks missing
the boat.

### Late Guests

-   Hosts should **not delay the boat** or spend the birthday
    troubleshooting late arrivals.
-   Assign a helper as the day-of guest contact.
-   If someone misses departure, direct them to the after-party.
-   Final guest wording should treat **11:45 AM as the arrival deadline**.

------------------------------------------------------------------------

## 3. Immediate Priorities

### Next 48 Hours

-   [x] **Email/text/call Aaron** with all outstanding boat questions.
-   [ ] Talk through this master plan with CayCay.
-   [x] Confirm which details must be known before the Partiful
    announcement.

### By End of This Week

-   [x] Receive/confirm Aaron's answers.
-   [x] Remove the optional 11:00 AM pre-boat meetup; early birds are on
    their own until the single official 11:45 AM boat meetup.
-   [ ] Send **one clean Partiful update** with confirmed logistics.
-   [ ] Make clear that space is limited and guests should update their
    RSVP if they cannot attend.

### This Weekend / Early Next Week

-   [ ] Reach out to **Sam, Kira, and Lynn** about helper roles.
-   [ ] Assign MC / crowd-cue role - likely **Sam or Kira**.
-   [ ] Assign day-of logistics/contact helper(s).
-   [ ] Assign photo/video lead(s).

### By Mid-September

All major decisions should be organized even if purchases happen later:

-   [ ] After-party selected.
-   [ ] Food/drink quantities calculated.
-   [ ] Shopping/purchasing owners assigned.
-   [ ] Supply transportation plan finalized.
-   [ ] Playlist owner selected and playlist underway.
-   [ ] Speaker confirmed + backup planned.
-   [ ] Decor plan finalized.
-   [ ] Cake plan finalized.
-   [ ] Photo/video owner finalized.
-   [ ] Guest payment process finalized.
-   [ ] Parking/rideshare instructions prepared.
-   [ ] Cleanup plan finalized.
-   [ ] Birthday activity/game options selected, if any.

------------------------------------------------------------------------

# 4. Aaron - Boat Questions

**Resolved for current planning.** Aaron logistics no longer block the
rest of the party plan. Confirmed or accepted planning decisions are
recorded below. Remaining caveats—ice, candles, exact host unloading,
and cleanup specifics—should be handled as practical day-of checks, not
as blockers.

## Current Resolution

-   **Guest arrival / boarding readiness:** **11:45 AM** at **Bass Tub,
    276 Jefferson Street, Berth #4**.
-   **Departure / cruise:** Booking begins at **12:00 PM** and runs to
    **2:00 PM**. Do not plan around late-arrival accommodations.
-   **Food and alcohol:** Outside food, BYOB, beer, Prosecco/champagne,
    hard liquor/tequila, and glass bottles are allowed based on Aaron's
    response and the listing evidence reviewed by the hosts.
-   **Cooler:** Provided onboard.
-   **Ice:** May be available but is not guaranteed. Check with Aaron
    the morning of the party and plan to bring ice if needed.
-   **Music:** Bluetooth stereo is available throughout the boat. A
    backup speaker is optional and probably unnecessary.
-   **Boat layout:** The listing confirms a large heated cabin with two
    dinettes and bench seating. Plan pizza, cake storage, extra supplies,
    and appropriate jackets/bags inside; drinks and the main social area
    centrally outside; views and photos around the outdoor perimeter.
-   **Cake and decor:** Aaron explicitly approved a birthday cake and
    simple decorations.
-   **Candles:** Not explicitly confirmed. Bring candles and a lighter,
    but ask day-of before using an open flame; candles are not a blocker.
-   **Cleanup:** Specific requirements were not discussed. Bring
    trash/recycling bags and leave the boat clean.
-   **Day-of contact / meeting point:** Aaron; **Bass Tub, 276 Jefferson
    Street, Berth #4**.
-   **Unloading:** Cart supplies to Berth #4. Exact dock access and early
    boarding were not promised; coordinate with Aaron upon arrival.
-   **Parking:** Pier 39 Garage is a convenient option about a 7-minute
    walk from the boat. It remains Gio's logistics base for the supply
    car and may be shared as an option—not a requirement—with guests.
-   **USCG rule:** No drugs of any kind, including cannabis, are allowed
    onboard. Include this briefly in guest communication.

## Boarding & Timing

-   [x] What time can the host/helper crew arrive?
-   [x] What time can guests begin boarding?
-   [x] Should all guests be at the dock by 11:45 AM?
-   [x] Does the boat depart exactly at 12:00 PM?
-   [x] Does the booked 12:00-2:00 PM window include boarding/setup
    time?
-   [x] What happens if someone arrives after departure?
-   [x] Is there anything guests need to do/sign/show before boarding?

## Food & Drinks

-   [x] Is outside food allowed?
-   [x] Can we bring Costco pizzas?
-   [x] Is BYOB allowed?
-   [x] Is beer allowed?
-   [x] Is Prosecco/champagne allowed?
-   [x] Is hard liquor/tequila allowed?
-   [x] Are **glass bottles** allowed?
-   [x] Are there restrictions on alcohol containers?
-   [x] Does the boat provide a cooler?
-   [x] How large is it / how much drink storage is available?
-   [x] Is ice provided?
-   [x] Are cups provided?
-   [x] Is there a refrigerator or other cold storage?
-   [x] Any food/drink items we should specifically avoid?

## Boat Amenities

-   [x] Is there a working Bluetooth/music speaker?
-   [x] How do guests connect to it?
-   [x] Should we bring our own backup speaker?
-   [x] Confirm bathroom access/capacity.
-   [x] Is there comfortable seating for roughly 40-50 guests?
-   [x] Are there indoor/covered areas if it gets cold or windy?
-   [x] Are there outlets/charging options?

## Cake & Decorations

-   [x] Can we bring a birthday cake?
-   [x] Are birthday candles allowed?
-   [x] If not, can we use flameless candles?
-   [x] What decorations are allowed?
-   [x] Are balloons allowed?
-   [x] Are banners/tape allowed?
-   [x] What attachment methods are safe for the boat?
-   [x] Are there decorations that are prohibited?

## Trash & Cleanup

-   [x] What are our cleanup responsibilities?
-   [x] Does the boat provide trash/recycling bins?
-   [x] Should we bring our own trash bags?
-   [x] Where does trash/recycling go when we dock?
-   [x] How much cleanup time do we have before leaving the boat?

## Day-of Logistics

-   [x] Best phone number for Aaron/day-of contact.
-   [x] Exact meeting/boarding landmark: Bass Tub, 276 Jefferson Street,
    Berth #4.
-   [x] Best place for hosts to unload supplies.
-   [x] Is short-term loading/unloading possible near the boat?
-   [x] Any parking/garage recommendation for guests?
-   [x] Any other instructions Aaron wants included in the guest
    message?

------------------------------------------------------------------------

# 5. People & Responsibilities

## Core Hosts

### CayCay

-   Birthday girl / primary social host.
-   Owns the **guest list and invitations**.
-   Decides who she wants at her birthday.
-   Should **not** be burdened with day-of logistics.

### Gio

-   Overall logistics / planning lead.
-   Coordinates Aaron, supplies, timeline, and master plan.
-   Helps host the party.
-   Should delegate day-of troubleshooting enough to actually enjoy the
    party.

## Helper Crew

### Sam

Potential: - MC / crowd cue - Setup - Guest coordination - Cleanup

### Kira

Potential: - MC / crowd cue - Setup - Guest coordination - Cleanup

### Lynn

Potential: - Setup - Guest coordination - Cake/birthday moment -
Photos/video - Cleanup

### Helper principle

Sam, Kira, and Lynn have already offered to help. Keep assignments
**light and specific**, rather than making anyone work the entire party.

**TODO:** - \[ \] Choose **Sam or Kira** as primary informal MC. - \[ \]
Choose backup MC. - \[ \] Choose day-of guest contact. - \[ \] Choose
cake owner. - \[ \] Choose photo/video lead(s). - \[ \] Choose
supply/setup helpers. - \[ \] Choose cleanup helpers.

------------------------------------------------------------------------

# 6. Food & Drinks

## Current Direction

Hosts plan to provide: - Pizza - Beer - Prosecco - Water - Potentially
tequila. Aaron's response/listing evidence confirms alcohol and glass
are allowed; hosts still need to decide quantities.

Guests: - Suggested **\$20 contribution** toward the boat. - May be
invited to **BYOB** if permitted. - Contribution should remain the
primary ask; guests should not feel required to both pay and bring
alcohol.

## Pizza

Current plan: **Four Costco pizzas**, served inside the heated cabin on
or around one dinette or another protected interior surface. This keeps
the boxes out of the outdoor social space and helps the pizza stay warm.

-   [x] Set current pizza quantity at four; revisit only if attendance
    changes materially.
-   [ ] Determine varieties.
-   [ ] Determine pickup time.
-   [ ] Assign purchaser/pickup person.
-   [x] Confirm outside food with Aaron.
-   [ ] Plan transport so pizza arrives hot/reasonably fresh.

## Drinks

Calculate later based on final RSVPs:

-   [ ] Beer quantity
-   [ ] Prosecco quantity
-   [ ] Water quantity
-   [ ] Tequila/liquor quantity if allowed
-   [ ] Non-alcoholic options
-   [ ] Ice
-   [ ] Cups
-   [ ] Napkins
-   [ ] Bottle opener/corkscrew if needed

### Cooler

-   Gio has a cooler available.
-   Boat provides a cooler.
-   Bring Gio's cooler only as an optional backup if useful.
-   Ice is not guaranteed; check with Aaron morning-of and bring ice if
    needed.

------------------------------------------------------------------------

# 7. Cake & Birthday Moment

This is the one intentionally planned moment during an otherwise
unstructured party.

## Goal

Have everyone gather, sing **Happy Birthday**, and celebrate CayCay with
a birthday cake.

-   [ ] Choose/order cake.
-   [x] Confirm cake is allowed onboard.
-   [ ] Assign one person to **pick up, transport, and protect the
    cake**.
-   [ ] Bring cake knife/server if needed.
-   [ ] Bring plates/forks/napkins.
-   [ ] Confirm candles/fire rules day-of before lighting anything.
-   [ ] Bring candles + lighter, but do not use an open flame without
    permission.
-   [ ] Use flameless candles if necessary.
-   [ ] Have Sam/Kira/MC quietly gather everyone when the moment feels
    right.
-   [ ] Have photo/video lead ready.

**Important:** No rigid scheduled minute is necessary. Let the MC find a
natural point during the cruise.

------------------------------------------------------------------------

# 8. Music

-   [x] Confirm boat Bluetooth/speaker with Aaron.
-   [ ] Bring backup portable speaker only if useful; the boat Bluetooth
    stereo is confirmed.
-   [ ] Choose playlist owner.
-   [ ] Build birthday/boat playlist.
-   [ ] Download playlist offline in case reception is poor.
-   [ ] Charge phones/speaker before leaving.
-   [ ] Make sure at least two people can access the playlist.

**Style:** Fun background/party music, not a formal DJ schedule.

------------------------------------------------------------------------

# 9. Decorations

Goal: **simple, festive, portable, easy to clean up.**

Possible direction: - Purple accents because purple is CayCay's favorite
color. - Birthday banner. - Small amount of boat-safe decor. - Avoid
anything that creates major setup/cleanup.

-   [x] Ask Aaron what is permitted.
-   [ ] Decide simple decor within Aaron's approval.
-   [ ] Assign purchaser.
-   [ ] Assign setup helpers.
-   [ ] Avoid loose items that can blow into the Bay.
-   [ ] Avoid glitter/confetti unless explicitly permitted - default is
    **no**.

------------------------------------------------------------------------

# 10. Photos & Video

The hosts should not have to remember to document the party.

-   [ ] Pick 1-2 friends as informal photo/video leads.
-   [ ] Capture:
    -   CayCay arriving/boarding
    -   Couple photos
    -   Friend/group photos
    -   Bay/boat views
    -   Birthday cake + singing
    -   One full-group photo
    -   Candid moments
-   [ ] Ask MC to cue the full-group photo before the cruise ends.
-   [ ] Have friends share photos afterward.

No hired photographer is currently planned.

------------------------------------------------------------------------

# 11. Activities / Games

Crowd already knows each other. **No name tags. No forced formal
icebreakers.**

Desired: optional, lightweight activities that fit naturally into a
two-hour boat party.

Potential ideas: - CayCay trivia - "How do you know CayCay?" quick
stories - Photo challenge - Fun group photo prompts - Small
conversational game

Avoid: - Rage Cage - King's Cup or anything messy/high-impact - Games
requiring lots of equipment - Anything disrespectful to the
boat/captain - Anything that dominates the two-hour cruise

-   [ ] Decide whether to prepare **one optional activity**.
-   [x] Ask Aaron if there are any restrictions relevant to games.

------------------------------------------------------------------------

# 12. Attire / What Guests Should Bring

Final Partiful update should include a short practical note.

Recommended: - **Dress in layers** - SF Bay can be colder/windier than
shore. - Comfortable, boat-friendly shoes. - Sunglasses. - Sunscreen. -
Optional snug/secure hat - loose hats can blow away. - Optional purple
clothing/accessory if CayCay likes the idea. - BYOB only if confirmed by
Aaron.

Do **not** turn this into a formal dress code.

------------------------------------------------------------------------

# 13. Transportation & Supplies

## Supply Transport

Gio's brother has a **foldable wheeled grocery/transport cart** that can
make moving supplies from the car to the boat much easier.

Use it for: - Drinks - Cooler - Decor - Cake supplies - Miscellaneous
party supplies

**Do not stack the cake somewhere unstable.**

-   [ ] Borrow brother's cart.
-   [x] Gio drives the supplies.
-   [x] Identify parking plan for Gio's supply car: Pier 39 Garage.
-   [ ] Determine whether someone can temporarily unload close to the
    dock.
-   [ ] Assign who physically moves supplies.
-   [ ] Keep a consolidated packing list so nothing stays in the car.

## Guest Transportation

-   [x] Record Pier 39 Garage as a convenient guest option about a
    7-minute walk away; do not present it as required parking.
-   [ ] Determine whether rideshare should be recommended.
-   [ ] Add concise transportation instructions to Partiful.
-   [ ] Encourage enough buffer to be physically at the meeting point by
    11:45.

------------------------------------------------------------------------

# 14. After-Party

**Not selected yet.**

Goal: - Nearby and easy after docking at 2:00 PM. - Able to handle a
potentially large group. - Low-friction transition from boat. - Guests
who miss the boat can be directed there.

-   [ ] Choose after-party bar/location.
-   [ ] Check hours.
-   [ ] Check group/capacity policy.
-   [ ] Determine whether reservation is needed.
-   [ ] Determine walking distance from dock.
-   [ ] Add to Partiful once confirmed.
-   [ ] Give late guests this location.

------------------------------------------------------------------------

# 15. Payments

Decision so far: - **\$20 suggested contribution** - Likely Venmo to
**CayCay**, because this is her birthday and most guests know her
directly. - Final timing is not yet locked.

Current preference: - Keep the party experience low-pressure. - People
may pay beforehand if they want. - A follow-up after the party may be
easiest, though people can forget.

-   [ ] Confirm CayCay's preferred Venmo/account.
-   [ ] Decide whether Partiful update asks for payment before or after.
-   [ ] Decide who sends payment reminder.
-   [ ] Track payments simply - no need to over-engineer.

------------------------------------------------------------------------

# 16. Cleanup

Goal: Leave the boat in excellent condition without Gio or CayCay doing
everything.

Bring: - **Heavy-duty trash bags** - Recycling bags if appropriate

Plan: - Encourage guests to throw things away as they go. - Near the
end, MC/helper can casually announce: - gather personal belongings -
dispose of trash - help with a quick cleanup - Helper crew performs
final sweep.

-   [x] Confirm boat trash/recycling setup with Aaron; specific setup was
    not discussed, so bring bags and leave the boat clean.
-   [ ] Buy/bring trash bags.
-   [ ] Assign final sweep helpers.
-   [ ] Remove all decorations.
-   [ ] Check for phones, jackets, sunglasses, bags, etc.
-   [ ] Confirm nothing is left onboard.

------------------------------------------------------------------------

# 17. Weather Plan

Do not overreact to forecasts a month out.

### \~10 Days Before

-   [ ] Check San Francisco/Fisherman's Wharf marine-area forecast.

### 3-5 Days Before

-   [ ] Recheck weather.
-   [ ] Confirm with Aaron if conditions create any concern.
-   [ ] Send clothing/weather guidance if necessary.

### Day Before

-   [ ] Final weather check.
-   [ ] Final guest reminder.

**Decision principle:** Normal SF chill/wind is manageable with layers.
Safety/cancellation decisions should be based on the operator/captain's
guidance.

------------------------------------------------------------------------

# 18. Partiful Communication

**Partiful is the single guest communication channel.**

Aaron's critical logistics are resolved. There is no formal early-bird
meetup to choose. Send the major update after the remaining payment,
transportation, and after-party details are ready.

## Main Update - Target: End of This Week

Should include:

-   🎉 Boat is officially booked.
-   **Saturday, October 3**
-   **Cruise: 12:00-2:00 PM**
-   **Official arrival / ready to board: 11:45 AM**
-   **Location: Bass Tub, 276 Jefferson Street, Berth #4, Fisherman's
    Wharf**
-   Boarding readiness: **11:45 AM**.
-   Boat leaves on time / don't be late.
-   Early-bird guidance: explore Fisherman's Wharf or grab a bite nearby
    independently, then meet everyone at the boat at 11:45 AM.
-   Suggested **\$20 contribution** + payment instructions.
-   Food/drink/BYOB details; outside food and BYOB are allowed.
-   Brief rule: no cannabis or other drugs are allowed onboard.
-   Clothing: layers, comfortable shoes, sunglasses.
-   Transportation/parking recommendation.
-   After-party if selected.
-   RSVP reminder:
    -   Space is limited.
    -   If someone can no longer attend, update RSVP so the spot can go
        to someone else.

## Final Reminder - Day Before

Keep short: - Arrival time - Exact meeting location - Departure
warning - Weather/layers - What to bring - BYOB if applicable -
Parking/rideshare - Day-of contact - After-party

------------------------------------------------------------------------

# 19. Master Shopping / Packing List

Quantities TBD.

## Food

-   [ ] Costco pizzas
-   [ ] Birthday cake
-   [ ] Cake plates
-   [ ] Forks
-   [ ] Napkins
-   [ ] Cake knife/server

## Drinks

-   [ ] Beer
-   [ ] Prosecco
-   [ ] Tequila/liquor - only if allowed
-   [ ] Water
-   [ ] Non-alcoholic drinks
-   [ ] Ice
-   [ ] Cups
-   [ ] Bottle opener/corkscrew

## Birthday

-   [ ] Candles/flameless candles
-   [ ] Lighter if permitted
-   [ ] Decorations
-   [ ] Banner

## Logistics

-   [ ] Cooler if needed
-   [ ] Brother's folding cart
-   [ ] Heavy-duty trash bags
-   [ ] Recycling bags
-   [ ] Backup speaker
-   [ ] Charging cable/power bank
-   [ ] Any tape/fasteners specifically approved for boat

------------------------------------------------------------------------

# 20. Day-of Checklist

## Before Leaving Home

-   [ ] Check weather.
-   [ ] Check with Aaron about ice; bring ice if it is not provided.
-   [ ] Confirm Aaron/day-of contact.
-   [ ] Charge phones.
-   [ ] Charge backup speaker.
-   [ ] Download playlist offline.
-   [ ] Confirm cake.
-   [ ] Confirm pizza.
-   [ ] Load drinks.
-   [ ] Load cooler/ice.
-   [ ] Load decor.
-   [ ] Load trash bags.
-   [ ] Load cart.
-   [ ] Check packing list.
-   [ ] Text helper crew if needed.

## At Fisherman's Wharf

-   [ ] Host/helper crew arrives first.
-   [ ] Contact Aaron.
-   [ ] Unload supplies.
-   [ ] Setup allowed decorations.
-   [ ] Set up drinks/food as permitted.
-   [ ] Helpers know their roles.
-   [ ] Guest contact monitors arrivals.
-   [ ] Guests gathered and ready to board by 11:45.
-   [ ] Boarding begins when Aaron allows.
-   [ ] Make sure CayCay is enjoying herself - not doing logistics.

## On Boat

-   [ ] Music running.
-   [ ] Drinks/food accessible.
-   [ ] Photos/candids happening naturally.
-   [ ] Birthday cake + Happy Birthday.
-   [ ] Full-group photo.
-   [ ] Optional activity only if the vibe calls for it.
-   [ ] No rigid schedule.

## Before Docking

-   [ ] MC/helper gives belongings/cleanup reminder.
-   [ ] Trash/recycling collected.
-   [ ] Decorations removed.
-   [ ] Final belongings sweep.
-   [ ] Cake/food/drink leftovers collected.
-   [ ] Transition guests toward after-party.

------------------------------------------------------------------------

# 21. Open Decisions

These are the remaining planning decisions, not forgotten tasks.

1.  **After-party**
2.  **Food quantities**
3.  **Drink quantities**
4.  **Playlist owner**
5.  **Simple decor selection**
6.  **Cake + cake owner**
7.  **MC: Sam or Kira**
8.  **Day-of guest contact**
9.  **Photo/video lead**
10. **Supply purchaser(s)**
11. **Supply driver/transport and unloading**
12. **Rideshare instructions**
13. **Payment timing + CayCay Venmo**
14. **Optional boat activity**
15. **Final Partiful wording**
17. **Morning-of ice check with Aaron**
18. **Day-of candle/open-flame permission**

------------------------------------------------------------------------

# 22. Dashboard Data Model - Phase 2

When an AI converts this Markdown into the shareable GitHub Pages
dashboard, the website should treat this document as the source of
truth.

Each task should support:

-   **Task**
-   **Status:** Not Started / Waiting / In Progress / Done
-   **Owner:** Gio / CayCay / Sam / Kira / Lynn / Other
-   **Due date**
-   **Priority:** Critical / High / Normal / Nice-to-have
-   **Dependency**
-   **Notes**

## Recommended Dashboard Sections

1.  **Today / Next Actions**
2.  **Countdown to October 3**
3.  **Event Snapshot**
4.  **Task Board**
5.  **Timeline**
6.  **Aaron / Boat Resolutions**
7.  **People & Roles**
8.  **Guest Communication**
9.  **Food & Drinks**
10. **Shopping / Packing**
11. **Day-of Plan**
12. **Open Decisions**

### Dashboard principle

The first screen should answer:

> **What do we need to do next, who owns it, and what is blocking it?**

Avoid turning the site into a public guest invitation. It is primarily a
**planning dashboard for Gio, CayCay, and trusted helpers**, with only
the information they need.

------------------------------------------------------------------------

# Definition of Done

The party is ready when:

-   Aaron's rules and boarding details are confirmed.
-   Guests have one clear Partiful message with accurate arrival
    information.
-   Guest list is within capacity.
-   Food, drinks, cake, music, and decor are covered.
-   Every physical item has an owner and transportation plan.
-   Sam/Kira/Lynn know their lightweight roles.
-   Guests know where and when to arrive.
-   Weather is checked close to the event.
-   Cleanup and late-arrival plans exist.
-   Gio and CayCay can board the boat without needing to actively manage
    logistics.

> **North Star:** By noon on October 3, the planning should disappear.
> CayCay gets to feel celebrated, Gio gets to enjoy the party with her,
> and the rest is just good music, friends, the Bay, and vibes.
