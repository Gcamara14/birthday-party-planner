...
const requiredFacts = [
  ...
  '276 Jefferson Street', 'Berth #4', 'Boat capacity', '55 people', 'Partiful guest limit',
...
const standardTasks = masterPlan.match(/^\s*-\s+\[[ xX]\]/gm) ?? []
const completedTasks = masterPlan.match(/^\s*-\s+\[[xX]\]/gm) ?? []
const inlineTasks = masterPlan.match(/\\\[ \\\]/g) ?? []
assert.equal(standardTasks.length, 211, 'The standard task inventory changed; audit task parsing and metadata.')
assert(completedTasks.length > 0, 'Expected source-backed completed tasks after the Aaron update.')
...
console.log(`Project source validation passed: 218 checkbox tasks (${completedTasks.length} completed) and boat-question categories are present.`)
